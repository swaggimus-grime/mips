...
use crate::psx::cd::disc::cache::CachedResult;
use crate::psx::cd::iso9660;
use crate::psx::graphics::gpu::VideoStandard;
...
impl SerialNumber {
    ...
    fn from_bin_name(bin: &[u8]) -> MipsResult<SerialNumber> {
        if bin.len() != 11 {
            return Err(MipsError::NoSerialNumber);
        }
...
pub fn extract_system_region(image: &mut dyn Image) -> MipsResult<Region> {
    ...
    let region = match license.as_ref() {
        ...
        _ => {
            ...
            return Err(MipsError::BadDiscFormat(m));
        }
    };
...
fn read_system_cnf(image: &mut CdCache) -> MipsResult<Vec<u8>> {
    ...
    if len > 1024 * 1024 {
        ...
        return Err(MipsError::BadDiscFormat(desc));
    }
