//! Basic MIPS assembler to generate PlayStation machine code in pure rust
    ...
    impl Instruction {
        ...
        pub fn bytes(&self, here: u32) -> u32 {
            match *self {
                ...
                La(..) => 8,
                Align(o) => super::pad_to_order(here, o),
                _ => 4,
            }
        }
    }
...
impl Assembler {
    ...
    pub fn assemble(&mut self, instructions: &[Instruction]) -> Result<u32, String> {
    ...
    /// them
    fn parse_labels(&mut self, instructions: &[Instruction]) -> Result<(), String> {
        let mut loc = self.location();

        for &i in instructions {
            match i {
                Global(name) => {
                    if self.globals.insert(name, loc).is_some() {
                        // Globals can't be redefined
                        return Err(format!("Global label '{}' is redefined", name));
                    }
                }
    ...
    fn label_address(&self, label: Label) -> Result<u32, String> {
        match label {
            Label::Global(l) => match self.globals.get(l) {
                Some(&v) => Ok(v),
            ...
            Label::Local(l, d) => {
                ...
                let v = match d {
                    ...
                    'b' => self
                        ...
                        .iter()
                        .rev()
                        .find(|&&(addr, name)| addr <= here && l == name),
                    // Search for the closest label *after* `here`
                    'f' => self
                        .locals
                        .iter()
                        .find(|&&(addr, name)| addr > here && l == name),
                    _ => return Err(format!("Unknown local label direction '{}'", d)),
                };
    ...
    fn assemble_instruction(&mut self, instruction: Instruction) -> Result<(), String> {
        match instruction {
            Sll(r0, r1, shift) => {
                self.emit_code(MachineCode::sub(0b00_0000).d(r0).t(r1).shift(shift))
            }
            Srl(r0, r1, shift) => {
                self.emit_code(MachineCode::sub(0b00_0010).d(r0).t(r1).shift(shift))
            }
            Sra(r0, r1, shift) => {
                self.emit_code(MachineCode::sub(0b00_0011).d(r0).t(r1).shift(shift))
            }
            Sllv(r0, r1, r2) => self.emit_code(MachineCode::sub(0b00_0100).d(r0).t(r1).s(r2)),
            Srlv(r0, r1, r2) => self.emit_code(MachineCode::sub(0b00_0110).d(r0).t(r1).s(r2)),
            Srav(r0, r1, r2) => self.emit_code(MachineCode::sub(0b00_0111).d(r0).t(r1).s(r2)),
            Jr(r0) => self.emit_code(MachineCode::sub(0b00_1000).s(r0)),

            Jalr(r0, r1) => self.emit_code(MachineCode::sub(0b00_1001).d(r0).s(r1)),
            Syscall(v) => self.emit_code(MachineCode::sub(0b00_1100).sys_brk(v)),
            Break(v) => self.emit_code(MachineCode::sub(0b00_1101).sys_brk(v)),
            Mfhi(r0) => self.emit_code(MachineCode::sub(0b01_0000).d(r0)),
            Mthi(r0) => self.emit_code(MachineCode::sub(0b01_0001).s(r0)),
            Mflo(r0) => self.emit_code(MachineCode::sub(0b01_0010).d(r0)),
            Mtlo(r0) => self.emit_code(MachineCode::sub(0b01_0011).s(r0)),
            Mult(r0, r1) => self.emit_code(MachineCode::sub(0b01_1000).s(r0).t(r1)),
            Multu(r0, r1) => self.emit_code(MachineCode::sub(0b01_1001).s(r0).t(r1)),
            Div(r0, r1) => self.emit_code(MachineCode::sub(0b01_1010).s(r0).t(r1)),
            Divu(r0, r1) => self.emit_code(MachineCode::sub(0b01_1011).s(r0).t(r1)),
            Add(r0, r1, r2) => self.emit_code(MachineCode::sub(0b10_0000).d(r0).s(r1).t(r2)),
            Addu(r0, r1, r2) => self.emit_code(MachineCode::sub(0b10_0001).d(r0).s(r1).t(r2)),
            Sub(r0, r1, r2) => self.emit_code(MachineCode::sub(0b10_0010).d(r0).s(r1).t(r2)),
            Subu(r0, r1, r2) => self.emit_code(MachineCode::sub(0b10_0011).d(r0).s(r1).t(r2)),
            And(r0, r1, r2) => self.emit_code(MachineCode::sub(0b10_0100).d(r0).s(r1).t(r2)),
            Or(r0, r1, r2) => self.emit_code(MachineCode::sub(0b10_0101).d(r0).s(r1).t(r2)),
            Xor(r0, r1, r2) => self.emit_code(MachineCode::sub(0b10_0110).d(r0).s(r1).t(r2)),
            Nor(r0, r1, r2) => self.emit_code(MachineCode::sub(0b10_0111).d(r0).s(r1).t(r2)),
            Slt(r0, r1, r2) => self.emit_code(MachineCode::sub(0b10_1010).d(r0).s(r1).t(r2)),
            Sltu(r0, r1, r2) => self.emit_code(MachineCode::sub(0b10_1011).d(r0).s(r1).t(r2)),
            Bgez(r0, l) => {
                let i = self.branch_target(l)?;

                self.emit_code(
                    MachineCode::op(0b00_0001)
                        .is_link(false)
                        .is_bgez(true)
                        .s(r0)
                        .imm_se(i),
            ...
            J(l) => {
                ...
                self.emit_code(MachineCode::op(0b00_0010).imm_jump(i));
            }
            Jal(l) => {
                let i = self.jump_target(l)?;

                self.emit_code(MachineCode::op(0b00_0011).imm_jump(i));
            }
            Beq(r0, r1, l) => {
                let i = self.branch_target(l)?;

                self.emit_code(MachineCode::op(0b00_0100).s(r0).t(r1).imm_se(i));
            }
            Bne(r0, r1, l) => {
                let i = self.branch_target(l)?;

                self.emit_code(MachineCode::op(0b00_0101).s(r0).t(r1).imm_se(i));
            }
            Blez(r0, l) => {
                let i = self.branch_target(l)?;

                self.emit_code(MachineCode::op(0b00_0110).s(r0).imm_se(i));
            }
            Bgtz(r0, l) => {
                let i = self.branch_target(l)?;

                self.emit_code(MachineCode::op(0b00_0111).s(r0).imm_se(i));
            }
            Addi(r0, r1, i) => {
                self.emit_code(MachineCode::op(0b00_1000).t(r0).s(r1).imm_se(i));
            }
            Addiu(r0, r1, i) => {
                self.emit_code(MachineCode::op(0b00_1001).t(r0).s(r1).imm_se(i));
            }
            Slti(r0, r1, i) => {
                self.emit_code(MachineCode::op(0b00_1010).t(r0).s(r1).imm_se(i));
            }
            Sltiu(r0, r1, i) => {
                self.emit_code(MachineCode::op(0b00_1011).t(r0).s(r1).imm_se(i));
            }
            Andi(r0, r1, u) => {
                self.emit_code(MachineCode::op(0b00_1100).t(r0).s(r1).imm(u));
            }
            Ori(r0, r1, u) => {
                self.emit_code(MachineCode::op(0b00_1101).t(r0).s(r1).imm(u));
            }
            Xori(r0, r1, u) => {
                self.emit_code(MachineCode::op(0b00_1110).t(r0).s(r1).imm(u));
            }
            Lui(r0, u) => {
                self.emit_code(MachineCode::op(0b00_1111).t(r0).imm(u));
            }
            Lb(r0, r1, i) => {
                self.emit_code(MachineCode::op(0b10_0000).t(r0).s(r1).imm_se(i));
            }
            Lh(r0, r1, i) => {
                self.emit_code(MachineCode::op(0b10_0001).t(r0).s(r1).imm_se(i));
            }
            Lwl(r0, r1, i) => {
                self.emit_code(MachineCode::op(0b10_0010).t(r0).s(r1).imm_se(i));
            }
            Lw(r0, r1, i) => {
                self.emit_code(MachineCode::op(0b10_0011).t(r0).s(r1).imm_se(i));
            }
            Lbu(r0, r1, i) => {
                self.emit_code(MachineCode::op(0b10_0100).t(r0).s(r1).imm_se(i));
            }
            Lhu(r0, r1, i) => {
                self.emit_code(MachineCode::op(0b10_0101).t(r0).s(r1).imm_se(i));
            }
            Lwr(r0, r1, i) => {
                self.emit_code(MachineCode::op(0b10_0110).t(r0).s(r1).imm_se(i));
            }
            Sb(r0, r1, i) => {
                self.emit_code(MachineCode::op(0b10_1000).t(r0).s(r1).imm_se(i));
            }
            Sh(r0, r1, i) => {
                self.emit_code(MachineCode::op(0b10_1001).t(r0).s(r1).imm_se(i));
            }
            Swl(r0, r1, i) => {
                self.emit_code(MachineCode::op(0b10_1010).t(r0).s(r1).imm_se(i));
            }
            Sw(r0, r1, i) => {
                self.emit_code(MachineCode::op(0b10_1011).t(r0).s(r1).imm_se(i));
            }
            Swr(r0, r1, i) => {
                self.emit_code(MachineCode::op(0b10_1110).t(r0).s(r1).imm_se(i));
            }
            Mfc0(r0, cop_r) => self.emit_code(
                MachineCode::op(0b01_0000)
                    .cop_opcode(0b0_0000)
                    .t(r0)
                    .cop_r(cop_r),
            ),
            Mtc0(r0, cop_r) => self.emit_code(
                MachineCode::op(0b01_0000)
                    .cop_opcode(0b0_0100)
                    .t(r0)
                    .cop_r(cop_r),
            ...
            // Pseudo instructions
            Nop => self.assemble_instruction(Sll(R0, R0, 0))?,
            Move(r0, r1) => self.assemble_instruction(Addu(r0, r1, R0))?,
            Li(r0, v) => {
                let hi = (v >> 16) as u16;
                let lo = v as u16;
            ...
            B(l) => self.assemble_instruction(Beq(R0, R0, l))?,
            Beqz(r0, l) => self.assemble_instruction(Beq(r0, R0, l))?,
            Bnez(r0, l) => self.assemble_instruction(Bne(r0, R0, l))?,
...
impl MachineCode {
    ...
    fn sys_brk(self, v: u32) -> MachineCode {
        MachineCode(self.0 | ((v & 0xf_ffff) << 6))
    }
    ...
    fn imm_jump(self, v: u32) -> MachineCode {
        MachineCode(self.0 | (v & 0x3ff_ffff))
    }
}
